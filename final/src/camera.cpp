...
k4a::device camera::init_kinect(k4a::capture &capture, k4a::transformation &k4aTransformation, k4a::calibration &k4aCalibration, int index)
{
  k4a::device Device;
  ...
  Device.get_imu_sample(imu_sample, std::chrono::milliseconds(100));
...
void camera::getAngel(k4a::device &device)
{
  ...
  if (imu_sample != NULL)
  {
    // cout << imu_sample->temperature << endl;
     __s16 angle[3] = {0};
     float imuAngle = -atan((float)imu_sample->acc_sample.xyz.x / powf(powf((float)imu_sample->acc_sample.xyz.z, 2) + powf((float)imu_sample->acc_sample.xyz.y, 2), 0.5)) * 180.f / PI;
     angle[0] = imuAngle;
     angle[1] = 0;
     angle[2] = 0;
